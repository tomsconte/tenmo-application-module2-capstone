...
private static final String API_BASE_URL = "http://localhost:8080/";
	...
	private void viewCurrentBalance() {
		BigDecimal account = accountService.getBalanceByUserId(currentUser.getUser().getId());
		System.out.println("Your current account balance is: $" + account);
		
	}

	private void viewTransferHistory() {
		accountService.displayTransferInformation(currentUser);
	}

	private void viewPendingRequests() {
		accountService.displayPendingTransfers(currentUser);
	}
	...
	private void requestBucks() {
		transferService.requestTransfer(currentUser);
	}
